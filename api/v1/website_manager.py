...
@web.route('/website_manager', methods=['GET', 'DELETE', 'PATCH'])
@auth.login_required
def website_manager():
...
def get_web_list() -> flask.Response:
    ...
    result = [{"name": i['name'], "port": i['port']} for i in web_list]
    return jsonify({"status": "OK", "msg": "Websites list", "result": result})
